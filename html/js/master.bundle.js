...
mainGradient.append('stop').attr('class', 'stop-right')
// .attr('stop-opacity', 0.5)
.attr('offset', '0%');

mainGradient.append('stop').attr('class', 'stop-left').attr('offset', '10%');

mainGradient.append('stop').attr('class', 'stop-left').attr('offset', '20%');

mainGradient.append('stop').attr('class', 'stop-right').attr('offset', '30%');

mainGradient.append('stop').attr('class', 'stop-right').attr('offset', '40%');

mainGradient.append('stop').attr('class', 'stop-left').attr('offset', '52%');

mainGradient.append('stop').attr('class', 'stop-left').attr('offset', '64%');

mainGradient.append('stop').attr('class', 'stop-right').attr('offset', '76%');

mainGradient.append('stop').attr('class', 'stop-left').attr('offset', '88%');

mainGradient.append('stop').attr('class', 'stop-right').attr('offset', '100%');

svg.append('rect').classed('filled', true).attr('x', 15).attr('y', 40).attr("filter", "url(#blur)").attr('height', innerHeight).attr('width', innerWidth);

var svgDefs2 = svg.append('defs');
var mainGradient2 = svgDefs2.append('linearGradient').attr('id', 'mainGradient2');
mainGradient2.append('stop').attr('class', 'stop-right2').attr('offset', '0');
mainGradient2.append('stop').attr('class', 'stop-left2').attr('offset', '5%');
svg.append('rect').classed('filled2', true).attr('x', 0).attr('y', 0).attr('height', innerHeight + 100).attr('width', innerWidth + 100).transition().duration(3800).ease(d3.easeQuadInOut).attr('transform', 'translate(1000, 0)');

var filter = svgDefs.append("filter").attr("id", "blur").append("feGaussianBlur").attr("stdDeviation", 7);
...
svg.append('rect').attr('fill', '#333333').attr('class', 'topLeftMask').attr('x', 0).attr('y', 0).attr('height', 50).attr('width', 50);
svg.append('rect').attr('fill', '#333333').attr('class', 'topRightMask').attr('x', innerWidth).attr('y', 0).attr('height', 50).attr('width', 50);
...
var render = function render(data) {
	...
	}).curve(d3.curveNatural).y0(15).y1(function (d) {
		return yScale(d.percent) + 20;
	});

	svg.append("path").data([data]).attr("class", "area").attr('fill', '#333333').attr("d", area);
	...
	path.attr("stroke-dasharray", totalLength + " " + totalLength).attr("stroke-dashoffset", totalLength).transition().duration(3000).ease(d3.easeQuadInOut).attr("stroke-dashoffset", 0);

	var lineGoal = svg.append('g').attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')').attr('class', 'lineGoal');
	lineGoal.append('line').attr('x1', 0).attr('y1', yScale(89.6)).attr('y2', yScale(89.6)).transition().duration(1000).attr('x2', innerWidth - 15);
	...
	g.selectAll("rect").data(data).enter().append("rect").attr('width', 2).attr('y', function (d) {
		...
		return xScale(mainValue(d));
	}).attr('class', 'lineDropping').transition().delay(function (d, i) {
		return i * 200;
	...
	dots.selectAll("dot").data(data).enter().append("circle").attr("r", 5).attr('opacity', 0).attr('cx', function (d) {
		return xScale(mainValue(d));
	}).attr('class', 'circles').transition().delay(function (d, i) {
		return i * 200;
	}).duration(1000).ease(d3.easeSin).attr('opacity', 1).attr('cy', innerHeight);

	var xAxisPercent = svg.append('g').attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')').attr('class', 'percents');
	xAxisPercent.selectAll("text").data(data).enter().append("text").attr('x', function (d) {
		return xScale(mainValue(d)) - 17;
	...
	if (data[0].dashed) {
		...
		path.attr("stroke-dashoffset", totalLength).attr("stroke-dasharray", dashArray).transition().duration(2000).attr("stroke-dashoffset", 0);
	}
};
